...
def main():
    ...
    while(1):
        ...
        if (tlx, tly, brx, bry) == (-1, -1, -1, -1):
            # Obtenemos la cara
            face_cascade = cv2.CascadeClassifier('.\src\haarcascade.xml')
            ...
            if cmtIniciado == 1:
                ...
                if oCmt.has_result:
                    ...
                    imgCara = img[oCmt.tl[1]:oCmt.bl[1], oCmt.tl[0]:oCmt.tr[0]]
                    cv2.imshow('frame', imgCara)
            else:
                oCmt.initialise(gray, (tlx, tly), (brx, bry))
                cmtIniciado = 1
             
        cv2.imshow('frame-color', img)
